...
class TFDistributionGaussianDiag(TFDistribution):
    ...
    def flat_params(self):
        ...
        if (len(mean_shape) == 2 and len(logstd_shape) == 1):
            mean_rows = tf.shape(input=mean_tf)[0]
    ...
    def logp(self, x):
        ...
        logp_tf = -0.5 * tf.reduce_sum(input_tensor=tf.square(diff_tf / self._std), axis=-1)
        logp_tf += -0.5 * self._dim * np.log(2.0 * np.pi) - tf.reduce_sum(input_tensor=self._logstd, axis=-1)
        return logp_tf
    ...
    def kl(self, other, eps=0):
        ...
        kl_tf = tf.reduce_sum(input_tensor=other.get_logstd() - self._logstd + (tf.square(self._std) + tf.square(self._mean - other.get_mean())) / (2.0 * other_var), axis=-1)
        kl_tf += -0.5 * self._dim
        return kl_tf

    def kl_reg(self):
        kl_tf = tf.reduce_sum(input_tensor=-self._logstd + 0.5 * (tf.square(self._std) + tf.square(self._mean)), axis=-1)
        kl_tf += -0.5 * self._dim
        return kl_tf
    ...
    def sample(self):
        shape_tf = tf.shape(input=self._mean)
    ...
    def param_reg_loss(self):
        ...
        err = tf.reduce_sum(input_tensor=tf.square(params_tf), axis=-1)
        loss = 0.5 * tf.reduce_mean(input_tensor=err)
    ...
    def _build_params(self, reuse):
        if (self._mean is None):
            ...
            self._mean = tf.convert_to_tensor(value=self._mean, dtype=tf.float32)

        if (self._logstd is None):
            self._logstd =  self._build_params_logstd(self._input, tf.shape(input=self._mean), "logstd", reuse)
        elif (isinstance(self._logstd, np.ndarray)):
            self._logstd = tf.convert_to_tensor(value=self._logstd, dtype=tf.float32)
    ...
    def _calc_entropy(self, logstd):
        ent_tf = tf.reduce_sum(input_tensor=logstd, axis=-1)
